        ...
        return (
            <sr-opt-read>
                ...
                <sr-opt-gp>
                    ...
                    <Slider min="12" max="30" icon={['icon-set-Reduce-font-20px', 'icon-set-increase-font-20px']} step="2" width={slider_width} value={ this.parse( this.props.option.fontsize == "" ? "18" : this.props.option.fontsize ) } onChange={ (v)=>this.changeFontsize(v) }/>
                </sr-opt-gp>
                <sr-opt-gp>
                    <sr-opt-label>字间距</sr-opt-label>
                    <Slider min="0" max="10" step="1" icon={['icon-set-narrow-wordspacing-20px', 'icon-set-enlarge-wordspacing-20px']} width={slider_width} value={ this.parse( this.props.option.custom.art.letterSpacing ) } onChange={ (v)=>this.changeStyle(v, "letterSpacing") }/>
                </sr-opt-gp>
                <sr-opt-gp>
                    <sr-opt-label>版面宽度</sr-opt-label>
                    <Slider min="720" max="1200" step="1" icontext={[700, 1200]} width={slider_width} value={ this.parse( this.props.option.layout == "" ? "1000" : this.props.option.layout ) } onChange={ (v)=>this.changeLayout(v) }/>
                </sr-opt-gp>
                <sr-opt-gp>
