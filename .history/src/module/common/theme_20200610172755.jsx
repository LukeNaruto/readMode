    ...
    render() {
        return (
            <sr-opt-themes onClick={ evt=> this.changeBgColor(evt) }>
                { this.props.themes.map( (theme,idx) => <sr-opt-theme title={this.props.labels[idx]} style={{backgroundColor: `rgba( ${theme} )`}} name={ this.props.names[idx] }>A</sr-opt-theme> )}
                {/* data-tooltip={ this.props.labels[idx] } data-tooltip-position="up" data-tooltip-delay="50" */}
            </sr-opt-themes>
        )
    }
