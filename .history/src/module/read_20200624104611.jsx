...
export default class ReadOpt extends React.Component {
    ...
    changeWhiteList(v){
        ...
        this.props.onChange && this.props.onChange( `whitelist_${this.props.option.whitelist}` );
    ...
    render() {
        const {hostname} = window.location;
        storage.Read(()=>{
            var str1 = this.state.whitelist.toString();
            var str2 = storage.read.whitelist.toString();
            ...
            console.log(22333,storage);
            
        })

        console.log(22,storage);
        ...
        return (
            <sr-opt-read>
                ...
                <sr-opt-gp>
                    <Switch width="100%" checked={ this.state.whitelist.includes(hostname) }
                        thumbedColor="#fff" trackedColor="#008eff"
                        label="当前网页自动开启阅读模式"
