...
storage.Read( () => {
    ...
    console.log('storage.Read--', local.Firstload(), local.Count(), ver.version);
    ...
    if ( local.Firstload() ) {
       ...
       !local.Count() && storage.GetRemote( "remote", ( result, error ) => {
        ...
        console.log( ver.version == storage.version && ver.patch != storage.patch);
...
browser.runtime.onMessage.addListener( function( request, sender, sendResponse ) {
    ...
    switch ( request.type ) {
        ...
        case msg.MESSAGE_ACTION.set_whitelist:
            const whitelist = request.value.list;
            console.log(whitelist);
            console.dir(storage.read.whitelist)
            storage.read.whitelist = whitelist;
            storage.Write( () => {
                
                console.log(storage.read.whitelist);
            })
                
            break;
        case msg.MESSAGE_ACTION.get_whitelist:
            const name = "xbread";
            browser.storage.local.get( [name], result => {
                const whitelist = result[name].read.whitelist;
