...
$('#addCurSite').on('click', function(){
  sendMessageToContent(msg.MESSAGE_ACTION.location,(res)=>{
    setList(res.hostname);
  });
})

$('input:radio[value="mode1"]').attr('checked','true');
function sendMessageToContent(options, cb){
  browser.tabs.query( { active: true, currentWindow: true }, tabs => {
    browser.tabs.sendMessage( tabs[0].id, msg.Add( options ), res => {
      cb && cb(res);
    })
  })
}
sendMessageToContent(msg.MESSAGE_ACTION.icon_code, res => {
  mode_switch = res;
