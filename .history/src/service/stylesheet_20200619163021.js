...
async function customFontFamily(){
    ...
    fontFamilyRes_   = await fontFamilyRes.json();
    console.log(fontFamilyRes_,fontFamilyRes_.bd.data[0].f);
    // var a = await fetch( fontFamilyRes_.bd.data[0].f );
    // console.log(a);
    fontFamilyRes_.bd.data.forEach(item => {
        const {t: family, f: ttf, id} = item;
        ...
        if ( $target.length == 0 ) {
            $( "head" ).prepend(`<style type="text/css" id="${id}_xb">${styles}</style>`);
        } else {
            $target.html( styles );
        }
    });
}
function fontfamily(items){
    return;
    return {
        "name" : "字体样式",
