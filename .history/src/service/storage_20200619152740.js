...
const name = "simpread",
    ...
    read   = {
        ...
        fap       : true,
        highlight : false,
        shortcuts : "CTRL CTRL",//"SPACE SPACE",
        toc       : false,
        toc_hide  : false,
        theme     : "xbth1",
        fontfamily: "default",
...
class Storage {
    ...
    */
    ...
    async GetRemote( type, callback ) {
        ...
        switch ( type ) {
            ...
            case "remote":
                ...
                fontResult   = await fontFamilyRes.json();
                break;
            case "origins":
                url = origins;
                break;
        ...
        try {
            ...
            result ? callback( {...result, fontResult} ) : callback( undefined, "error" );
        } catch ( error ) {
    ...
    Statistics( type, service, state = "write" ) {
        console.log(1,'Statistic--s', type, service, state);
    ...
    Restore( key ) {
        ...
        console.log( "current site object is 1", key, curori )
    ...
    /**
     * iconCode set/get, iconCode not import/export
     * 
     * @param {int}   iconCode
     */
    iconCode(callback, code ) {
        if ( typeof code == 'number' ) {
            browser.storage.local.set( { ["iconCode"] : code }, () => {
                callback && callback();
            });
        } else {
            browser.storage.local.get( ["iconCode"], result => {
                callback && callback(result);
            });
        }
    }
