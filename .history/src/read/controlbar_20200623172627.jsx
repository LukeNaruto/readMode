...
export default class ReadCtlbar extends React.Component {
    ...
    componentDidMount() {
        browser.runtime.onMessage.addListener( ( request, sender, sendResponse ) => {
            if ( request.type == msg.MESSAGE_ACTION.export ) {
                console.log( 1,"controlbar runtime Listener", request );
    ...
    render() {
        const Controlbar = <Fap items={ [ "样式"] } 
            ...
            onOpen={ ()=> this.onPop( "open" ) } 
            onClose={ ()=> this.onPop( "close" ) }
            onAction={ (event, type)=>this.onAction(event, type ) }
            onDispatchStorage={()=>{storage.Setcur( storage.current.mode );console.log(999,storage)}}>
            <ReadOpt option={ this.state.storage.current } onChange={ (t,c)=>this.onChange(t,c)}/>
            {/* <Actionbar items={ readItems } onAction={ (type)=>this.onAction(undefined, type ) }/>
            <Sitebar />
            <Pluginbar /> */}
        </Fap>
        return (
            <sr-rd-crlbar style={{ "zIndex": "2" }}>{/*  class={ this.props.show ? "" : "controlbar" } */}
                { Controlbar }
            </sr-rd-crlbar>
        )
    }
