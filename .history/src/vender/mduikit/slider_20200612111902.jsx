...
export default class Slider extends React.Component {
    ...
    onTextChange( mode ) {
        ...
        if(mode){
            value >= max ? value=max : value += step;
        }else{
            value <= min ? value=min : value -= step;
        }
    ...
    onChange( event ) {
        ...
        $state.css({ ...style.state, ...style.state_focus });
        // this.refs.input.value = event.target.value;
        $(this.refs.range).attr('title', event.target.value);
    ...
    componentDidMount() {
        ...
        $(this.refs.range).attr('title', this.props.value);
    ...
    render() {
        ...
        {tooltip, icons, icontexts} = this.props;
        let mintext,maxtext,minIcon, maxIcon;
        if(icons){
            minIcon = icons[0];
            maxIcon = icons[1];
        }else if(icontexts){
        ...
        return (
            <slider style={ style.root }
                ...
                <span className={`fap-tool iconfont ${minIcon}`} onClick={()=>this.onTextChange(false)}>{mintext}</span>
                ...
                <span className={`fap-tool iconfont ${maxIcon}`} onClick={()=>this.onTextChange(true)}>{maxtext}</span>
