...
export default class Fap extends React.Component {
    ...
    panelBgEventHandler( event ) {
        ...
        if ( event.target.tagName.toLowerCase() == "panel-bg" ) {
            ...
            this.setState({...this.state, renderBg: false});
    ...
    panelRender( $panelbg ) {
        if ( $panelbg.length > 0 ) {
            const style = { ...this.style };
            this.setState({...this.state, renderBg: true});
    ...
    render() {
        ...
        panelBg  = <panel-bg ref="bg" style={style.panel_bg}></panel-bg>;
