        ...
        cssinjs = () => {
            ...
            const
                ...
                line: {
                    ...
                    backgroundImage: 'linear-gradient(to right, #007eff, #00c0ff)',
...
export default class Slider extends React.Component {
    ...
    onTextChange( mode ) {
        let   istrue  = true, value = this.refs.range.value;;
        const style   = { ...this.style },
            //   $target = $( event.target ),
            //   $state  = $target.next().find( "text-field-state" ),
        ...
        $(this.refs.range).attr('data-tooltip', value);
        ...
        if ( !istrue ) {
            ...
            this.refs.range.value = value;
            // this.refs.input.value = event.target.value;
            $(this.refs.range).attr('title', value);
            this.lineWidth( value );
            this.props.onChange && this.props.onChange( value );
        } */
    }
    ...
    onChange( event ) {
        ...
        $state.css({ ...style.state, ...style.state_focus });
        // this.refs.input.value = event.target.value;
        $(this.refs.range).attr('data-tooltip', event.target.value);
    ...
    componentDidMount() {
        ...
        $(this.refs.range).attr('data-tooltip', this.props.value);
    ...
    render() {
        ...
        return (
            <slider style={ style.root }
                ...
                <span className={'fap-tool'} onClick={()=>this.onTextChange(false)}>123</span>
                <sl-group style={ style.group }>
                    <input ref="range" type="range" min={this.props.min} max={this.props.max} step={this.props.step} onChange={ evt=> this.onChange(evt) } onMouseUp={ evt=>this.onMouseUp(evt)}  data-tooltip-position="up" data-tooltip-delay="50" />
                    <line ref="line" style={ style.line }></line>
                </sl-group>
                <span className={'fap-tool'} onClick={()=>this.onTextChange(true)}>321</span>
