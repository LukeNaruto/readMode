...
export default class AC extends React.Component {
    ...
    onDropdownClick( event ) {
        if ( event.target.dataset.state == "close" ) {
            ...
            this.setState({ ...this.state, name : this.state.name, items: this.props.items, icon_: 'icon-Upward-px' });
            event.target.dataset.state = "open";
        } else {
            this.onTextChangeBlur(event);

            this.setState({ ...this.state, name : "", items: [], icon_: 'icon-down-px' });
    ...
    onDropdownChange( name, value ) {
        ...
        this.setState({ ...this.state, title : this.getActiveTitle(value), name });
        this.refs.dropdown.dataset.state = "close";
