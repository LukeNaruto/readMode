...
Object.assign( context.focus.menu,      menu, { id: "focus",     "title" : "聚1焦模式" });
...
console.log(9999, context);

/**
 * Listen contextMenus message
 */
function onClicked( callback ) {
    console.log(1,'dispatch--',callback);
 ...
 */
function createAll(code) {
    if(code === -1) return;
    browser.contextMenus.create({ "type": "radio" });
    ...
    console.log(223,storage.option.menu,context, context);
}
...
function update( type, enabled = true  ) {
    if ( context.read.id ) {
        const title = type == "read" ? "阅读模式" : "当前网页不支持阅读模式";
        browser.contextMenus.update( context.read.id, { title, enabled } );
    }
}
