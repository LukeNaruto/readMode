...
function fontFamily( family ) {
    console.log('fontFamily-----',$( "#read_container_" ), family);
    
    $( "#read_container_" ).get(0).style.setProperty("--font-family", family == "default" ? "" : family)
    // css( "--font-family", family == "default" ? "" : family );
}
...
function fontSize( value ) {
    ...
    value ? $( "html" ).attr( "style", `--fsize: ${value - 18}px!important;height:100%;${html_style_bal}` ) : $( "html" ).attr( "style", html_style_bal );
}
...
async function customFontFamily(fonts){
    fonts.forEach(font => {
        const {t: family, f: ttf, id} = font;
        const $target = $( "head" ).find( `style#${id}_xb` );
        var styles = `
            @font-face {
                font-family: "${family}";
        ...
        if ( $target.length == 0 ) {
            $( "head" ).append(`<style type="text/css" id="${id}_xb">${styles}</style>`);
            $( "head" ).append(`<link rel="preload" href="${ttf}" as="font" type="font/ttf" crossorigin="anonymous" />`);
        } else {
            $target.html( styles );
        }
