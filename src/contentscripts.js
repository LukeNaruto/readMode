 ...
 */
storage.Read( () => {
    console.log('storage.Read---',blacklist(),isLazyload());
    if ( blacklist() ) {
        $( "style" ).map( ( idx, item ) => {
...
function blacklist() {
    ...
    console.log( "current site is blacklist", is_blacklist,storage,storage.option)
...
browser.runtime.onMessage.addListener( function( request, sender, sendResponse ) {
    ...
    switch ( request.type ) {
        case msg.MESSAGE_ACTION.set_read_mode:
            const {whitelist, auto} = request.value;
            storage.read.whitelist = whitelist;
            if(auto !== undefined) storage.read.auto = auto;
            storage.Write( () => {
                console.log(storage);
            })
            window.postMessage({"message": msg.MESSAGE_ACTION.reload_whitelist}, '/');
            break;
        case msg.MESSAGE_ACTION.read_active:
            console.log('--------',pr, $('.simpread-read-root').length);
            const active = $('.simpread-read-root').length > 0;
            sendResponse(active);
            break;
...
function readMode() {
    ...
    console.log(JSON.parse(JSON.stringify(storage)),JSON.parse(JSON.stringify(pr)));
        ...
        if ( state ) {
            ...
            const idx = storage.read.whitelist.indexOf(hostname);
            if(~idx){
                storage.read.whitelist.splice(idx,1);
            }
    ...
    if  ( enabled ) {
        ...
        switch ( storage.current.site.name ) {
            ...
            default:
                ...
                // pr.state == "temp"    && pr.current.site.html != "" && readMode();
                break;
        }
...
function getCurrent( mode ) {
    if ( mode && storage.VerifyCur( mode ) ) {
        ( !pr || !pr.Exist() ) && pRead();
        console.log( "current site object is 2", mode, JSON.parse(JSON.stringify(pr)) )
...
function browserAction( is_update ) {
    ...
    browser.runtime.sendMessage( msg.Add( msg.MESSAGE_ACTION.browser_action, { code: pr.state !== "adapter" ? -1 : 0 , url: window.location.href } ));
